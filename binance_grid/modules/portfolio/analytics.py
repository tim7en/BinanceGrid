...
from .manager import PortfolioManagerSnapshot
...
def summarize_walkforward_snapshots(snapshots: Sequence[PortfolioManagerSnapshot]) -> WalkForwardAnalytics:
    if not snapshots:
    ...
    equity_curve = np.array([snapshot.total_equity for snapshot in snapshots], dtype=float)
    ...
    active_grids = np.array(
        [sum(asset_snapshot.active_grid_levels for asset_snapshot in snapshot.asset_snapshots.values()) for snapshot in snapshots],
        dtype=float,
    )
    step_grid_crosses = np.array(
        [sum(asset_snapshot.grid_levels_crossed_in_step for asset_snapshot in snapshot.asset_snapshots.values()) for snapshot in snapshots],
        dtype=float,
    )
    cumulative_grid_crosses = np.cumsum(step_grid_crosses)
    step_fills = np.array(
        [sum(asset_snapshot.fills_in_step for asset_snapshot in snapshot.asset_snapshots.values()) for snapshot in snapshots],
        dtype=float,
    )
    ...
    step_breakouts = np.array(
        [sum(1 for asset_snapshot in snapshot.asset_snapshots.values() if asset_snapshot.breakout_triggered) for snapshot in snapshots],
        dtype=float,
    )
    cumulative_breakouts = np.cumsum(step_breakouts)
    step_fake_breakouts = np.array(
        [sum(1 for asset_snapshot in snapshot.asset_snapshots.values() if asset_snapshot.fake_breakout_triggered) for snapshot in snapshots],
        dtype=float,
    )
